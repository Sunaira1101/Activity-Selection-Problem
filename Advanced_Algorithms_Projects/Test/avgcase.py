...
def measure_time(size_of_array, is_avg_case=False):
    if is_avg_case:
...
# To measure the avg-case time complexity
avg_execution_times = []
for size in input_sizes:
    execution_time = measure_time(size, is_avg_case=True)
    avg_execution_times.append(execution_time)

# Write avg_execution_times to a text file
with open('avg_case_execution_times.txt', 'w') as file:
    for time_val, sizes in zip(avg_execution_times, input_sizes):
        file.write(f'{sizes} = {time_val:.16f}\n')

avg_total_execution_time = sum(avg_execution_times)
